...
When using the API, job offers are published to *JobRoom* (https://www.job-room.ch) and *JobDesk* (http://pilot.job-desk.ch).
...
The following chapters explain more precisely some of these concepts and provide examples in how to create, read, edit and delete job offers.
...
The following codes are available to set the country in the _location_ object :
